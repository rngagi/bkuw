...
- The current product includes audio import/playback and in-app recording, corpus CSV and XeLaTeX/Overleaf/PDF export. Cloud sync, collaboration, mobile clients, production signing, notarization, and background automatic upload remain excluded. The explicitly planned user-triggered Publish corpus website workflow is the only planned cloud deployment exception.
...
- Prepare releases only through `pnpm release:prepare -- <version>`; do not edit canonical version files or push version tags by hand. Published version state belongs to GitHub Releases, not duplicated README or plan text.
