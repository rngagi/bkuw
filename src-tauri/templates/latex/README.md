...
2. Upload the `*-overleaf.zip` generated by bkuw. If only this extracted folder is available, create a ZIP whose root contains `main.tex`.
3. Use XeLaTeX if Overleaf does not select it automatically.

1. 在 Overleaf 選擇 **New Project → Upload Project**。
2. 上傳 bkuw 產生的 `*-overleaf.zip`。若只保留這個解壓縮資料夾，請重新壓縮，並讓 `main.tex` 位於 ZIP 根目錄。
