...
- GitHub Actions 在 `main`／pull request 的 Windows x64 與 macOS Apple Silicon jobs 執行 checks、tests 與 release-mode desktop E2E，但不建立或上傳 installer artifacts，也不建立 macOS Intel 產物。成功的 trusted `main` push CI 會觸發 release planner；只有 exact commit 同時遞增 package、Cargo、Cargo lock 與 Tauri version，且對應 tag 尚不存在時，才於 Windows／macOS jobs 建置並暫存 NSIS／DMG。受限 `contents: write` 的 final job 驗證檔名與 SHA-256，最後才在 exact SHA 建立 tag 與含自動 changelog 的 Draft Release。`publish-draft` module 可安全更新指向相同 commit 的既有草稿；manual recovery 可重用指定 release run 的 installer artifacts，無須重新打包。發布前保留人工確認閘門。
