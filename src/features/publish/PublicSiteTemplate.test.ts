...
const corpus = {
  ...
  entries: [
    { id: "e1", slug: "guo", primaryForm: "guò", notes: null, forms: [{ writingSystemId: "primary", text: "guò" }, { writingSystemId: "ipa", text: "kuɔ" }], senses: [{ id: "s1", partOfSpeech: "verb", gloss: "cross", definition: "move over", semanticDomain: "motion", images: [], audio: [], examples: [] }], relations: [] },
    { id: "e2", slug: "ata", primaryForm: "ata", notes: null, forms: [{ writingSystemId: "primary", text: "ata" }], senses: [{ id: "s2", partOfSpeech: "noun", gloss: "person", definition: null, semanticDomain: null, images: [], audio: [], examples: [] }], relations: [] },
  ],
};
...
describe("published website template", () => {
  ...
  it("folds diacritics for search, preserves display text, deep-links, toggles theme, and shows optional info", async () => {
    ...
    expect(document.querySelectorAll(".entry-item")).toHaveLength(1);
    expect(document.querySelector(".entry-headword")?.textContent).toBe("guò");
    fireEvent.click(document.querySelector<HTMLButtonElement>(".entry-item")!);
    expect(location.hash).toBe("#/entry/guo");
    expect(document.querySelector(".entry-title")?.textContent).toBe("guò");

    const theme = document.querySelector<HTMLButtonElement>("#theme-button")!;
    expect(document.documentElement.dataset.theme).toBe("light");
    expect(theme.getAttribute("aria-label")).toBe("Use dark theme");
    fireEvent.click(theme);
    expect(document.documentElement.dataset.theme).toBe("dark");
    expect(theme.getAttribute("aria-label")).toBe("Use light theme");
    expect(localStorage.getItem("bkuw-theme")).toBe("dark");
